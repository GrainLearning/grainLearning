...
It would be interesting to have a digital equivalent material to further investigate, using numerical simulations such as the discrete element method (DEM), how such material would behave under other mechanical constraints. To do so, the first step is defining a contact model governing interactions between grains in DEM. Then, using GrainLearning one can calibrate or infer the values of the contact model parameters such that the mechanical response observed in the real-world experiment is the closest to the one obtained in the DEM simulation.

Making abstraction of this idea, GrainLearning's use can be extended to other kind of models or *Dynamical systems* that can be other simulation frameworks such as FEM, CFD, LBM, and even other techniques such as agent based modelling. In the same vein, the framework is not exclusive for granular materials.
