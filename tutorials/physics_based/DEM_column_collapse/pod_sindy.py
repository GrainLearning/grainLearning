...
# ----------------------------
def simulate_and_reconstruct(model, U_r, A0, t_eval):
...
def visualize_velocity_magnitude(X, X_pred, shape, time_index):
    ux_t, uy_t = unpack_velocity(X[:, time_index], shape)
    ...
    plt.savefig(f"velocity_magnitude_comparison_{time_index}.png")
...
def main():
    ...
    # Visualize a mid-time slice of velocity magnitude (true vs pred)
    visualize_velocity_magnitude(X, X_pred, shape, time_index=t.shape[0]//2)
    ...
    num_modes_train = min(10, r_train)
    ...
    X_test_pred = simulate_and_reconstruct(model_train, U_r_train[:, :num_modes], A0_test, t_eval=t)

    # Error metrics on test set
    print_error_metrics(X, X_test_pred)

    # Visualize a mid-time slice of velocity magnitude (true vs pred) for test set
    visualize_velocity_magnitude(X, X_test_pred, shape, time_index=t.shape[0]//2)
