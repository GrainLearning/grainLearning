...
def test_model_output_shape():
    ...
    test_input_sequence = np.random.normal(size=(batch_size, window_size, input_shapes['num_input_features']))
    test_contacts = np.random.normal(size=(batch_size, input_shapes['num_params']))

    output = model({'inputs': test_input_sequence,
                    'params': test_contacts})
...
def test_get_pretrained_model(config_test):
    ...
    for path_to_model in path_to_model_test:
        ...
        assert train_stats.keys() == {'sequence_length', 'num_input_features',
...
def test_predict_macroscopics():
    ...
    predictions_1 = predict.predict_batch(model, data['test'], train_stats, config, batch_size=1)
    ...
    predictions_2 = predict.predict_batch(model, data_padded['test'], train_stats_2, config, batch_size=2)
    ...
    with pytest.raises(ValueError):
        predict.predict_batch(model, data_padded, train_stats, config, batch_size=2)
